...
> **📐 좌표계 규칙**: 좌표계는 **ROS 표준(REP-103)** 을 따릅니다.  
> **X = 전방(Forward), Y = 좌측(Left), Z = 상방(Up)**  
> 이는 모바일 로봇, 지상 로봇에서 가장 널리 사용되는 표준 좌표계입니다.  
>
> **🧭 Yaw 회전 및 방향 정의**:  
> - **Yaw (θ)**: Z축 기준 회전 각도, **반시계방향(CCW)이 양(+)**  
> - **동서남북 매핑 예시**: 전역 +X = 북쪽, +Y = 서쪽, -X = 남쪽, -Y = 동쪽  
